...
`custom_season_mapping`默认按照`[str] keyword: [int] season`格式填写；如果还需要保留部分标题文本，则可以写成：
...
```
ANi/BLEACH 死神 千年血战篇-诀别谭-/[ANi] BLEACH 死神 千年血戰篇 - S02E18 [1080P][Baha][WEB-DL][AAC AVC][CHT].mp4
```

文件子目录不会变，因为Jellyfin / Emby支持不同Season的TV分开存放在不同目录下，会自动合并。

未设置`reserve_keywords`时，`custom_season_mapping`会自动剔除文件名中的关键词，然后在集数前加上对应的Season。设置了`reserve_keywords`后，会用该值替换命中的关键词。
